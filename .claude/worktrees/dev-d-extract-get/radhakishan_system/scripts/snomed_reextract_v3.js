...
// Load relationships from BOTH India Extension AND International Edition
// India Extension has branded drugs; International has generic Clinical Drug parents
for (const relFile of [
  ...
  for (const line of fs.readFileSync(relFile, "utf8").split("\n")) {
    const c = line.split("\t");
...
const cvMap = new Map();
// Concrete values only exist in India Extension (branded drugs have the strengths)
...
// ===== MAIN =====
console.log(
  "\nRe-extracting formulations for drugs with 0 or restored-only formulations...",
);

// Only process the v2 files (which already have correct SNOMED data for most drugs)
// We only re-extract for drugs that have 0 SNOMED formulations
for (const file of [
  "formulary_data_ABDM_FHIR_v2.json",
  "formulary_data_ABDM_FHIR_generics_v2.json",
]) {
  ...
  for (const drug of drugs) {
    ...
    // Only process drugs that have no SNOMED-sourced formulations
    const hasSnomedForms = (drug.formulations || []).some((f) =>
      (f.indian_brands || []).some(
        (b) => b.verified_on && b.verified_on.includes("SNOMED"),
      ),
    );
    if (hasSnomedForms) continue; // Already has SNOMED data, skip

    // Is this a combo drug?
    const isCombo =
      drug.generic_name.includes("+") || drug.generic_name.includes(" and ");

    // Find Clinical Drug concepts — search base code + alt salt codes
    const codesToSearch = [drug.snomed_code];
    if (drug._alt_substance_codes)
      codesToSearch.push(...drug._alt_substance_codes);

    const allCDs = new Set();
    for (const code of codesToSearch) {
      const cds = ingredientToCDs.get(code);
      if (cds) cds.forEach((cd) => allCDs.add(cd));
    }
    if (!allCDs.size) {
      console.log(
        "  NO CDs: " +
  ...
  // Write back to same _v2 file (only affected drugs were modified)
...
// Orphans not touched by v3 — already in v2
