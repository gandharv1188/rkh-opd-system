...
- `02_architecture/adrs/ADR-004-datalab-webhooks-over-polling.md` — commits the adapter to a webhook-first + poll-fallback pattern (user's 2026-04-20 preference). Captures the `webhook_url` field + shared-secret plaintext auth + 5xx/timeout retry semantics verified in `10_handoff/document_ocr_flow.md §13.4`. Receiver endpoint deferred to DIS-097-extended in Epic D.
